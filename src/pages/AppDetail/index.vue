		...
		methods: {
			...
			async addShopCart() {
			   ...
			   //2.需要知道这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
				try {
					//1.在点击加入购物车按钮的时候，将参数带给服务器（发请求），通知服务器加入购物车的产品是谁
					await this.$store.dispatch('addOrUpdateShopCart', {
						skuId: this.$route.params.skuId,
						skuNum: this.skuNum
					})
					//如果成功，路由跳转，在路由跳转的时候要将产品的信息带给下一级的路由组件c
					//简单的数据skuNum,通过query形式给路由组件传递过去
					//产品信息的数据skuInfo,通过会话存储（本地存储/会话存储，一般存的是字符串）
