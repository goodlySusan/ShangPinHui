...
<script>
	...
	export default {
		...
		computed: {
			//mapGetters里面的写法：传递数组，因为getters计算没有划分模块
		...
		methods: {
			...
			removeCategoryName() {
				...
				//带给服务器的参数是可有可无的，undefined时，不会带给服务器
			...
			changeOrder(flag){
				...
				//综合还是价格
				let originName = this.searchParams.order.split(':')[0]
				//箭头上还是下
				let originArrow = this.searchParams.order.split(':')[1]
				//一个新的order属性值
				let newOrder = ''
				//如果点的是综合
				if(flag == originName){
					newOrder = `${originName}:${originArrow=='desc'?'asc':'desc'}`
				}else{
					newOrder = `${flag}:${'desc'}`
				}
